...
def get_slot_machine_spin(rows, clos, symbols):
    ...
    columns = []         ## represent the values in our column
    for _ in range(rows):
        column = []
...
def deposit():     ##function 
    while True:
        amount = input("Amount that you want to Deposit $")
        if amount.isdigit():  ## ansering a deposit 
            amount = int(amount)
            if amount > 10:  # cheking th eamount 
                break
            else:
...
def get_bet():
    while True:
        amount = input("Amount that you want to Bet on each line? $")
        if amount.isdigit():  
            amount = int(amount)
